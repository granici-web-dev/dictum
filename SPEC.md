...
- `issues.json` — схема в `/decompose`; дополнительно к схеме: `estimate: "S|M|L"` (S ≤ 2 ч, M ≤ полдня, L ≤ день) и `test_hint` (одна строка, как проверить) — поля взяты из практики Task Master. Инварианты, которые проверяет `app/validate.py`: идентификаторы уникальны, каждый `depends_on` существует, в графе нет циклов, зависимость не указывает в более позднюю фазу (иначе фаза не собирается в самостоятельный кусок работы)
...
- Ретраи: `max_retries=2` SDK Anthropic (сетевые ошибки, 408/409/429/5xx, экспоненциальный backoff). Ответ без единого `<file>`-тега или не прошедший проверку стадии (`decompose` → `check_issues`) → один ремонтный повтор: в диалог добавляется ответ модели и список претензий. Предыдущий ответ обязательно идёт в истории: без него стадия генерирует всё заново и идентификаторы перестают быть стабильными, чего требует `/decompose`. Если и второй ответ не проходит — `StageError` со списком проблем; локальный прогон сохраняет вторую попытку в `outputs/<stage>.raw.md` и возвращает `1`, файлы стадии на диск не пишутся. Ответ с чужим набором файлов не чинится: это не претензия к содержимому, а сорванный контракт. Ответ со `stop_reason != end_turn`, с чужим набором файлов или с двумя блоками на один путь → `StageError` с полем `raw` (текст ответа), без повтора. Токены и длительность в `StageResult` суммируются по обоим вызовам. Содержимое файла нормализуется: ровно один перевод строки в конце. Невалидный JSON от `decompose` → один повтор с сообщением об ошибке валидации через `user_edit` (P1-04).
