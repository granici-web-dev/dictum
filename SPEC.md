...
## 6. Trello
- Одна доска на команду, `TRELLO_BOARD_ID` в .env. Клиент не пользуется `raise_for_status`, а поднимает своё исключение со статусом, методом и путём без параметров: текст `raise_for_status` несёт полный URL, а в нём ключ и токен. Публикация закрыта тем же флагом `ALLOW_LIVE_API`, что и вызовы модели: мусорные карточки на командной доске — такой же необратимый расход, как оплаченный токен.
