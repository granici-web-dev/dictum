...
- Ретраи Trello (дополнение к §7, которое там не описано, потому что относится не к модели): 429 повторяется до трёх попыток с паузой из `Retry-After`, иначе 1 с с удвоением; 5xx повторяется только на GET. Повтор POST после 5xx запрещён: запрос мог дойти и потерять ответ, а вторая такая же карточка хуже упавшего прогона.
