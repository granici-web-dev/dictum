...
- `publish.json` — маппинг issue_id → card_id/url; отложенные скоупы лежат там же под своим `scope_id`
...
- Зависимости: строка «Depends on: I-003, I-007» в description + attachment-ссылка на карточку. Карточки создаются в порядке зависимостей, а не в порядке файла: ссылка требует, чтобы карточка-зависимость уже существовала, а в файле зависимость может стоять ниже зависящего. Цикла не будет, его исключает §5.
