...
## 6. Trello
- Одна доска на команду, `TRELLO_BOARD_ID` в .env. Публикация закрыта тем же флагом `ALLOW_LIVE_API`, что и вызовы модели: мусорные карточки на командной доске — такой же необратимый расход, как оплаченный токен.
...
- Карточка с маркером, у которой не хватает чеклиста DoD или ссылок на зависимости, дособирается: оборвавшийся прогон успевает создать карточку раньше, чем наполнить её, и без досборки она осталась бы недоделанной навсегда. В выводе это «уже есть, дособрана».
