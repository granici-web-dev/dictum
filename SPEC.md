...
- Порядок карточек в списке задаётся `pos` по порядку issue в файле, а не по порядку публикации: публикуем по зависимостям, и без этого `I-010` лёг бы выше `I-006`, что читается как сбитая нумерация.
