...
# Тариф claude-sonnet-5: $2 за миллион токенов входа, $10 за миллион выхода. Сверено по
# https://platform.claude.com/docs/en/about-claude/pricing 20.09.2026; назначенное на 01.09.2026
# повышение до $3 / $15 не состоялось, и оба замера ниже были посчитаны по нему — то есть по
# тарифу другой модели. Тарифы меняются: следующий, кто сюда посмотрит, знает дату сверки.
#
# Два замера разбора, и оценка идёт прямой через них обоих. Прогон 54f48ea8e0bc45c7, 19.09.2026:
# 2640 секунд записи дали 22 643 токена на вход и 20 439 на выход, то есть $0.25. Прогон
# fdbfda84c895f6b7, 20.09.2026: 204 секунды дали 7483 и 8518, то есть $0.10. Вторая точка
# опровергла первую линию: у разбора есть постоянная часть — системный промпт и размышление
# модели, — и линия из нуля по одной точке короткую запись не завышала, а занижала вчетверо.
SHORT_REVIEW_SECONDS = 204
SHORT_REVIEW_DOLLARS = 0.10
LONG_REVIEW_SECONDS = 2640
LONG_REVIEW_DOLLARS = 0.25
...
def review_price(seconds: int) -> str:
    """Прямая через оба замера: постоянная часть около $0.09 и около $0.004 за минуту записи."""
...
def price_line(seconds: int) -> str:
    ...
    обещала $0.02 за разбор и промахнулась в 12 раз, а линия по одному замеру обещала $0.02 за
    четырёхминутную запись, которая стоила $0.10. Оба раза мимо было в дешёвую сторону.
    """
    lines = [
