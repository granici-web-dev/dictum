...
# Кому версия инструмента позволяет его звать, словами API (`allowed_callers`). `direct` — модель
# зовёт поиск сама; `code_execution_*` — из песочницы кода, то есть с динамической фильтрацией.
SearchCaller = Literal[
    "direct", "code_execution_20250825", "code_execution_20260120", "code_execution_20260521"
]


class WebSearch(BaseModel):
    """Предел поисков стадии и то, кто её зовёт, свой на каждый режим ресёрча (SPEC §7, P3-11).
    ...
    # Прямой вызов вместо динамической фильтрации, включённой по умолчанию версии инструмента
    # (SPEC §7): фильтрующий режим отбирает результаты кодом в песочнице, и до модели они не
    # доходят. Значение лежит здесь, а не строкой посреди вызова, потому что это свойство
    # объявленного инструмента и меняется оно решением, а не по ходу дела.
    callers: tuple[SearchCaller, ...] = ("direct",)
